...
def generate_realistic_user_ids(num_orders, num_users, existing_orders=None):
    ...
    # Use only the actual generated users (1 to num_users)
    available_users = list(range(1, num_users + 1))
    
    # 20% new customers (from the generated users 1 to num_users)
    new_customers = min(int(num_orders * 0.2), num_users)
    if new_customers > 0:
        user_ids.extend(random.sample(available_users, new_customers))
    
    # 60% repeat customers (from available users, weighted toward recent users)
    repeat_customers = int(num_orders * 0.6)
    
    # Weight toward more recent user IDs (simulating active customers)
    # Recent users (last 20% of available users) get higher probability
    recent_user_count = max(1, int(len(available_users) * 0.2))
    recent_user_range = available_users[-recent_user_count:]
    ...
    if recent_selections > 0 and len(recent_user_range) > 0:
        recent_user_ids = np.random.choice(recent_user_range, min(recent_selections, len(recent_user_range)), p=recent_weights)
        user_ids.extend(recent_user_ids.tolist())
    
    # Remaining repeat customers from available users
    remaining_repeat = repeat_customers - len([uid for uid in user_ids if uid in recent_user_range])
    if remaining_repeat > 0:
        # Use remaining available users
        remaining_users = [uid for uid in available_users if uid not in user_ids]
        if len(remaining_users) > 0:
            remaining_weights = np.ones(len(remaining_users))
            remaining_weights = remaining_weights / remaining_weights.sum()
            remaining_user_ids = np.random.choice(remaining_users, min(remaining_repeat, len(remaining_users)), p=remaining_weights)
            user_ids.extend(remaining_user_ids.tolist())
    
    # 20% occasional customers (random from available users)
    occasional = num_orders - len(user_ids)
    if occasional > 0:
        # Use any remaining available users
        remaining_users = [uid for uid in available_users if uid not in user_ids]
        if len(remaining_users) > 0:
            occasional_user_ids = random.sample(remaining_users, min(occasional, len(remaining_users)))
            user_ids.extend(occasional_user_ids)
        else:
            # If we've used all users, just repeat from available users
            occasional_user_ids = random.choices(available_users, k=occasional)
            user_ids.extend(occasional_user_ids)
...
def generate_realistic_email_enhanced(name, signup_date, customer_tier):
    ...
    if year < 2020:
        # Older users: 30% creative, 70% name-based
        creative_ratio = 0.3
    elif customer_tier in ['Gold', 'Platinum']:
        # Premium customers: 70% creative, 30% name-based
        creative_ratio = 0.7
    else:
        # Modern regular users: 60% creative, 40% name-based
        creative_ratio = 0.6
